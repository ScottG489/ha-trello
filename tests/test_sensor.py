...
from custom_components.trello.sensor import async_setup_entry
...
async def test_sensor_setup_entry(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.trello.sensor.TrelloClient"), patch(
        "custom_components.trello.sensor.TrelloSensor"
    ) as mock_trello_sensor, patch(
        "custom_components.trello.sensor.TrelloDataUpdateCoordinator",
        autospec=True,
    ):
